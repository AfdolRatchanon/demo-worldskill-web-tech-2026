# Frontend-Simple — Pages Reference (สำหรับนักเรียนท่องจำ)

แอปเป็น React + Vite + axios + react-router-dom มี **4 หน้า** แยกตาม role
Backend อยู่ที่ `http://localhost:8080/api` (ตั้งใน [src/api.js](src/api.js))

## โครงร่วมที่ทุกหน้าใช้ (ต้องจำ)
- **[src/api.js](src/api.js)** — axios instance + interceptor แนบ `Authorization: Bearer <token>` ให้อัตโนมัติทุก request (อ่าน token จาก `localStorage` ใหม่ทุกครั้ง)
- **[src/auth.js](src/auth.js)** — `saveToken()` / `getUser()` / `removeToken()`
  - `getUser()` ถอด JWT (`atob` ส่วนกลาง) ได้ `{ id, username, role, full_name, candidate_code }`
- **[src/App.jsx](src/App.jsx)** — routing + `ProtectedRoute` (ถ้า `role` ไม่ตรง → เด้งไป `/login`)
...
## 1) Login — [src/pages/Login.jsx](src/pages/Login.jsx)
...
## 2) CandidatePage — [src/pages/CandidatePage.jsx](src/pages/CandidatePage.jsx)
...
## 3) JudgePage — [src/pages/JudgePage.jsx](src/pages/JudgePage.jsx)
...
## 4) ManagerPage — [src/pages/ManagerPage.jsx](src/pages/ManagerPage.jsx)
...
> ดูรายละเอียด request/response ของแต่ละ endpoint ได้ที่ [backend-simple/API_REFERENCE.md](../backend-simple/API_REFERENCE.md)
